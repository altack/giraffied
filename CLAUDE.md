...
- **`System.Tags` is merge-on-add, replace-on-replace.** ADO treats JSON-Patch `op: "add"` on `/fields/System.Tags` as a *merge* — the value is added to the existing list, not used to overwrite it. So sending `value: "Foo"` against a work item already tagged `Bar` leaves it tagged `Bar; Foo`. To set the tag list (including clearing it via `value: ""`), emit `op: "replace"`. Microsoft's docs split this across two examples ("Add Tag to Work Item" uses `add`, "Update Work Item Tags" uses `replace`) and it's easy to miss the distinction. Special-cased in `patchWorkItemFields` in `endpoints.ts`.
